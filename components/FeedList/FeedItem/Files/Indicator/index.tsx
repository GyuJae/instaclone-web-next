...
const Indicator: React.FC<IProps> = ({totalCount, currentIndex}) => {
  const list = useMemo(() => {
    return new Array(totalCount).fill(0).map((item, index) => {
      ...
      return (
        <div key={key} className={cls('h-[6px] w-[6px] rounded-full', currentIndex === index ? 'bg-blue-500' : 'bg-gray-400')}/>
      )
    })
  ...
  return (
    <div className='flex items-center justify-center space-x-1'>{list}</div>
  )
}
